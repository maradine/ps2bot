	...
	public TwitterListener(final PircBotX bot, final String channel, Properties props) {
		...
		StatusListener listener = new StatusListener() {
			...
			public void onStatus(Status status) {
				bot.sendMessage(channel, "@" + status.getUser().getScreenName() + ": " + status.getText());
			}
				...
				switch(token) {
					case "add":
						if (scan.hasNext()) {
							...
							setFilter();
							bot.sendMessage(channel, "Added " + name + " to twitter feeds.");
						}
						break;
					case "remove":
						if (scan.hasNext()) {
							...
							usersToFollow.add(name.toLowerCase());
							setFilter();
							bot.sendMessage(channel, "Removed " + name + " from twitter feeds.");
						}
						break;
					default: event.respond("I'm not sure what you asked me.  Valid commands are \"add\" and \"remove\".");
					break;
				}
			}
		}
