...
public class StatCollectionEngine implements Runnable {
	...
	public StatCollectionEngine(PircBotX bot, Properties props) {
		this.bot = bot;
		this.props = props;
		...
		while (true) {
			try {
				...
				if (onSwitch) {
					...
					try {
						HashMap<String, Integer> results = KillCollectorEnhanced.collectKills(timeout, props);
